 ...
 * 
 * This script generates static pages for sample businesses to test the system.
 * Run this to see what your business pages will look like.
 */
...
const sampleBusinesses = [
    {
        id: 1,
        name: "LA Fitness Sherman Oaks",
        slug: "la-fitness-sherman-oaks",
        address: "14500 Ventura Blvd, Sherman Oaks, CA 91403",
        phone: "(818) 789-1234",
        website: "https://www.lafitness.com",
        description: "Full-service fitness center with modern equipment, group classes, and personal training. Open 24/7 with state-of-the-art cardio and strength training equipment.",
        status: "active",
        ...
        business_tags: [
            ...
            { tags: { name: "Cardio Equipment", tag_type: "amenity" } },
            { tags: { name: "Strength Training", tag_type: "service" } }
        ]
    },
    {
        id: 2,
        name: "Joe's Pizza Encino",
        slug: "joes-pizza-encino",
        address: "16001 Ventura Blvd, Encino, CA 91436",
        phone: "(818) 555-0123",
        website: "https://www.joespizzaencino.com",
        description: "Authentic New York-style pizza made with fresh ingredients and traditional recipes. Family-owned since 1985, serving the best pizza in the San Fernando Valley.",
        status: "active",
        categories: { name: "Restaurants", slug: "restaurants" },
        business_tags: [
            { tags: { name: "Encino", tag_type: "location" } },
            { tags: { name: "New York Style", tag_type: "specialization" } },
            { tags: { name: "Family Owned", tag_type: "specialization" } },
            { tags: { name: "Delivery", tag_type: "service" } },
            { tags: { name: "Takeout", tag_type: "service" } },
            { tags: { name: "Cash Only", tag_type: "payment" } }
        ]
    },
    {
        id: 3,
        name: "Studio City Auto Repair",
        slug: "studio-city-auto-repair",
        address: "12345 Ventura Blvd, Studio City, CA 91604",
        phone: "(818) 555-0456",
        website: null,
        description: "Professional auto repair and maintenance services. ASE certified technicians with over 20 years of experience. Specializing in European and Japanese vehicles.",
        status: "active",
        categories: { name: "Auto Services", slug: "auto" },
        business_tags: [
            { tags: { name: "Studio City", tag_type: "location" } },
            { tags: { name: "ASE Certified", tag_type: "specialization" } },
            { tags: { name: "European Cars", tag_type: "specialization" } },
            { tags: { name: "Japanese Cars", tag_type: "specialization" } },
            { tags: { name: "Oil Change", tag_type: "service" } },
            { tags: { name: "Brake Service", tag_type: "service" } },
            { tags: { name: "Credit Cards Accepted", tag_type: "payment" } }
        ]
    },
    {
        id: 4,
        name: "Mary's Beauty Salon",
        slug: "marys-beauty-salon",
        address: "9876 Laurel Canyon Blvd, North Hollywood, CA 91605",
        phone: "(818) 555-0789",
        website: "https://www.marysbeautysalon.com",
        description: "Full-service beauty salon offering haircuts, coloring, styling, and spa treatments. Experienced stylists specializing in modern and classic looks.",
        status: "active",
        categories: { name: "Beauty & Personal Care", slug: "beauty" },
        business_tags: [
            { tags: { name: "North Hollywood", tag_type: "location" } },
            { tags: { name: "Haircuts", tag_type: "service" } },
            { tags: { name: "Hair Coloring", tag_type: "service" } },
            { tags: { name: "Spa Treatments", tag_type: "service" } },
            { tags: { name: "Appointments Required", tag_type: "service" } },
            { tags: { name: "Walk-ins Welcome", tag_type: "service" } }
        ]
    },
    {
        id: 5,
        name: "Valley Home Services",
        slug: "valley-home-services",
        address: "5432 Reseda Blvd, Reseda, CA 91335",
        phone: "(818) 555-0321",
        website: "https://www.valleyhomeservices.com",
        description: "Licensed contractors specializing in home repairs, remodeling, and maintenance. Over 15 years of experience serving the San Fernando Valley with quality workmanship.",
        status: "active",
        categories: { name: "Home Services", slug: "home-services" },
        business_tags: [
            { tags: { name: "Reseda", tag_type: "location" } },
            { tags: { name: "Licensed Contractors", tag_type: "specialization" } },
            { tags: { name: "Home Repairs", tag_type: "service" } },
            { tags: { name: "Remodeling", tag_type: "service" } },
            { tags: { name: "Free Estimates", tag_type: "service" } },
            { tags: { name: "Emergency Service", tag_type: "service" } }
        ]
    }
];
...
// Business page template (same as in generate-static-pages.js)
...
// Helper functions (same as in generate-static-pages.js)
...
async function generateTestPages() {
    console.log('🚀 Generating test static pages...');
    ...
    try {
        ...
        console.log(`📝 Generating ${sampleBusinesses.length} sample business pages`);
        
        // Process each business
        for (let i = 0; i < sampleBusinesses.length; i++) {
            const business = sampleBusinesses[i];
            console.log(`📄 Generating page ${i + 1}/${sampleBusinesses.length}: ${business.name}`);
            
            // Generate related businesses (other sample businesses)
            const relatedBusinesses = sampleBusinesses
                .filter(b => b.id !== business.id && b.categories?.name === business.categories?.name)
                .slice(0, 3);
            
            // Add tags to related businesses
            relatedBusinesses.forEach(relatedBusiness => {
                relatedBusiness.business_tags = relatedBusiness.business_tags || [];
            });
            
            // Generate HTML
            const html = generateBusinessPage(business, sampleHours, relatedBusinesses);
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <url>
    <loc>https://sanfernandovalley.xyz/</loc>
    <lastmod>${new Date().toISOString().split('T')[0]}</lastmod>
    <changefreq>weekly</changefreq>
    <priority>1.0</priority>
  </url>
${sampleBusinesses.map(business => `  <url>
    <loc>https://sanfernandovalley.xyz/business/${business.slug}</loc>
    <lastmod>${new Date().toISOString().split('T')[0]}</lastmod>
    <changefreq>weekly</changefreq>
    <priority>0.8</priority>
        ...
        console.log('🎉 Test static page generation completed!');
        console.log(`📁 Generated ${sampleBusinesses.length} business pages in /business/ directory`);
        console.log('🗺️  Generated sitemap.xml');
        console.log('\n📋 Next steps:');
        console.log('1. Open /business/la-fitness-sherman-oaks.html in your browser');
        console.log('2. Check the generated pages to see how they look');
        console.log('3. Run "npm run build" to generate pages from your actual database');
        
    } catch (error) {
        console.error('💥 Error during test page generation:', error);
        process.exit(1);
    }
}
...
module.exports = { generateTestPages };
